...
For($i=1; $i -le 5; $i++)
{
    ...
    #Don't run a DataServer
    #startProcess -dir .\AsyncAsDesigned.PerfDataServer -cmdArgs "run", "--configuration Release", "--no-build", "$i", "$guid"
    #Start-Sleep -Milliseconds 25
}


$appProcess = startProcess -dir .\AsyncAsDesigned.PerfAppServer -cmdArgs "run", "--configuration Debug", "--no-build", "sync", "5", "$guid"

#The first 4 tokens are really fast
# See the time it takes for new Threads to be assigned